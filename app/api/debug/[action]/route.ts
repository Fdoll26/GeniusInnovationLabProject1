...
import { generateDebugReport, listDebugRuns, runFromStep, type RunFromStepRequest } from '../../../../debug/debug-api';
...
export async function POST(request: Request, { params }: { params: Promise<{ action: string }> }) {
  try {
    ...
    if (action === 'run-from-step') {
      await requireSession();
      const body = (await request.json().catch(() => ({}))) as RunFromStepRequest;
      try {
        const data = await runFromStep(body);
        return NextResponse.json(data);
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        const status = message.includes('required') || message.includes('Invalid') || message.includes('not found in STEP_SEQUENCE') ? 400 : 500;
    ...
    if (action === 'generate-report') {
      await requireSession();
      const body = (await request.json().catch(() => ({}))) as { runId?: string; sessionId?: string };
      try {
        const pdfBuffer = await generateDebugReport({ runId: body.runId, sessionId: body.sessionId });
        const filename = `debug-report-${Date.now()}.pdf`;
        ...
        const message = error instanceof Error ? error.message : String(error);
        if (message.includes('no synthesized output available')) {
          return NextResponse.json(
            {
              error: 'Cannot generate report: no synthesized output available.',
              missingParts: ['OpenAI synthesized report', 'Gemini synthesized report']
            },
            { status: 422 }
          );
        }
